<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        <h1 class="m-0 text-2xl font-bold leading-tight text-gray-900 dark:text-white">{{ i18n.t('proposals.title') }}</h1>
        <p class="mt-4 mb-0 max-w-4xl text-sm text-gray-500 dark:text-dark-400">
          {{ i18n.t('proposals.subtitle') }}
        </p>
      </div>
      <n-button quaternary :loading="loading" title="Refresh proposals" @click="fetchProposals">
    ...
    <n-card size="small" :bordered="true">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <span class="text-xs text-gray-500 dark:text-dark-400 sm:ml-auto">
          {{ total }} proposals{{ statusFilter !== 'all' ? ` · ${statusFilter}` : '' }}
        </span>
      </div>
    </n-card>
    ...
    <n-spin :show="loading">
      ...
      <n-empty
        ...
        description="No proposals match this filter"
      >
        <template #extra>
          <n-button quaternary @click="setStatusFilter('all')">{{ i18n.t('proposals.showAll') }}</n-button>
        </template>
      </n-empty>
      <n-empty
        v-else
        description="No agent changes waiting for approval"
      >
        <template #extra>
          <div class="flex flex-wrap justify-center gap-2">
            <n-button type="primary" @click="router.push('/dashboard/knowledge')">
              <template #icon><div class="i-tabler-database" /></template>
              {{ i18n.t('nav.knowledge') }}
            </n-button>
            <n-button quaternary @click="router.push('/dashboard/agents')">
              <template #icon><div class="i-tabler-robot" /></template>
              {{ i18n.t('nav.agents') }}
            </n-button>
          </div>
        </template>
...
function parseStatusQuery(value: unknown): string {
  ...
  if (status === 'open' || status === 'approved' || status === 'rejected' || status === 'all') return status
  return 'open'
}
...
const stats = computed(() => [
  { label: i18n.t('proposals.statAll'), value: counts.value.all, filter: 'all' },
  { label: i18n.t('proposals.statOpen'), value: counts.value.open, filter: 'open' },
  { label: i18n.t('proposals.statApproved'), value: counts.value.approved, filter: 'approved' },
  { label: i18n.t('proposals.statRejected'), value: counts.value.rejected, filter: 'rejected' },
])
    ...
    render: (row) =>
      h('div', { class: 'flex min-w-0 flex-wrap items-center gap-2' }, [
        ...
        row.type
          ...
          : null,
        h(NTag, { size: 'small', type: 'info', bordered: false }, { default: () => scopeTag(row) }),
      ]),
  },
  {
    title: 'Slug',
    ...
    ellipsis: { tooltip: true },
    render: (row) =>
      h('span', { class: 'font-mono text-xs text-gray-500 dark:text-dark-400' }, [
        h('span', { class: 'mr-1 text-gray-400' }, actionLabel(row)),
        row.slug,
      ]),
  },
  {
...
async function fetchProposals() {
  ...
  try {
    ...
    const data = await apiFetch<ProposalPage>(`/v1/proposals?${params.toString()}`)
    proposals.value = data.proposals ?? []
    total.value = data.total ?? 0
    page.value = data.page ?? 1
    pageSize.value = data.pageSize ?? pageSize.value
    counts.value = data.counts ?? { all: 0, open: 0, approved: 0, rejected: 0 }
    error.value = ''
  } catch (err) {
